...
def start():
    ...
    if action == "1":
        ...
        print("Dans un royaume lointain vivait un soldat fan d'un rappeur: Sefyu. Alors agacé d'entendre en boucle J'peux manger du cassoulet halal en même temps que le mafé. Le roi le  fit bannir dans une forêt gardé par un être puissant que peux connaisait")
...
class PlayerRPG:
    ...
    def event(self):
        visited = ["Je jure je suis déja passé par la", "Paris c'est loin", "Je sortirai d'ici pour ceux qui mange pas de porc"]
        position = self.perform_move()
        print("Current position:", position)

        if position in self.visited_positions:
            print(random.choices(visited))
            return
        else:
        ...
        if position == "feu de camp":
            ...
            print("Besoin de quelque chose? J'ai tous, de l'illégal aussi!!")
            self.buy_merchant()
        elif position == "goblin":
            print("Une tête chauve un et nez pointu comme une fléchette sortent d'un buisson")
            self.combat("gobelin")


        elif position == "ogre":
            print("Des pas lourds et une jupe fleurit, un frison parcours l'échine")
            self.combat("ogre")



        elif position == "dragon":
            print("Un lézard qui crache le feu comme un dragon est en face")
            self.combat("dragon")
            ...
            print("T'as plus rien maintenant bouffon")
    ...
    def buy_merchant(self):
        ...
        if choice == "1" and self.merchant_inventory["potion_force"] > 0:
          ...
          print("Si mes articles sont pas bon tu peux dégager")
        else:
            print("Error")
    ...
    def afficher_statut_joueur(self):
        print("\033[92m Santé : {}\033[92m ".format(self.HP))
